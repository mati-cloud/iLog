...
pub struct ParsedFileProvider {
    parser_config: Arc<ParserConfig>,
}

impl ParsedFileProvider {
...
impl LogProvider for ParsedFileProvider {
    async fn start(&self, tx: mpsc::Sender<LogEntry>) -> Result<()> {
        info!("Starting ParsedFileProvider with {} sources", self.parser_config.sources.len());
